...
# Using  Decimal for the float value to avoid error while comparing in budgets
import threading
import time
...
def dashboard(request):
    ...
    checkbudget(request)
    check_emi_pending(request)
    EMI_schedule(request)
...
def process_emi_payments():
    ...
    for emi in emis:
        alert_message=f"Today is the payment day for next installment of EMI payment of {emi.amount}.The EMI ends on {emi.end_date}."
        if alert_message and not Alert.objects.filter(user=emi.user, message=alert_message).exists():
            Alert.objects.create(
            user=emi.user,
            message=alert_message )
            
def EMI_schedule(request):
    thread = threading.Thread(target=process_emi_payments, daemon=True)
    thread.start() 
...
def check_emi_pending(request):
    ...
    for emi in em_is_due:
        if emi.next_payment_date <= today and emi.next_payment_date <= emi.end_date:
            alert_messg=f"Your schedule for  EMI payment of Rs.{emi.amount} ends today. Please confirm payment in your EMIs page."
...
def alerts_list(request):
    ...
    return render(request, 'alerts_list.html', {'alerts': alerts,'display_type':'alerts_page'})
...
def generate_report(request):
    ...
    for emi in EMI.objects.filter(user=request.user):
        emi_data.append([emi.amount, emi.frequency, emi.start_date.strftime('%d-%m-%Y'),emi.end_date.strftime('%d-%m-%Y'),emi.last_payment_date,emi.next_payment_date,emi.description])
    ...
    if report_type == "i":
        ...
        Report.objects.create(user=request.user,report_type="overbud")
        messages.success(request,message="Report generated.")
        ...
        Report.objects.create(user=request.user,report_type="Monthly")
        messages.success(request,message="Report generated.")
        ...
        Report.objects.create(user=request.user,report_type="all")
        messages.success(request,message="Report generated.")
...
def reports_list(request):
    ...
    return render(request, 'alerts_list.html', {'reports': reports,'display_type':'reports_page'})
...
def add_goal(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            goal.user = request.user  # Set the logged-in user
            goal.save()
            messages.success(request, 'Goal has been added.')
            return redirect('goal_list') 
         # Redirect to the goallist view/dashboard 
         # here goal_list is the name value from urls.py of app
    else:
        form = GoalForm()
    return render(request, 'add_forms.html', {'goal_form': form, 'form_type': 'goal'})
...
def goal_list(request):
    ...
    deadline = request.GET.get('deadline')
        ...
        #  __gte  is for greater than or equal to
    if deadline:
        goals = goals.filter(deadline__lte=deadline)   
         # __lte  is for less than or equal to
...
def update_goal(request, goal_id):
    goal = get_object_or_404(Goal, id=goal_id, user=request.user)  # Ensure the user owns the income
    if request.method == 'POST':
    ...
    return render(request, 'add_forms.html', {'goal_form': form, 'form_type': 'update_goal'})
